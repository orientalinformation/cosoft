...
class Products extends Controller
{
    ...
    public function getProductViewModel($id) 
    {
        ...
        if ($count > 0) {
            ...
            foreach ($products as $key => $pr) {
                ...
                if ($pr->ID_SHAPE == SPHERE || $pr->ID_SHAPE == CYLINDER_CONCENTRIC_STANDING || $pr->ID_SHAPE == CYLINDER_CONCENTRIC_LAYING || $pr->ID_SHAPE == PARALLELEPIPED_BREADED || $pr->ID_SHAPE == CYLINDER_STANDING_3D || $pr->ID_SHAPE == CYLINDER_CONCENTRIC_STANDING_3D || $pr->ID_SHAPE == CYLINDER_CONCENTRIC_LAYING_3D || $pr->ID_SHAPE == PARALLELEPIPED_BREADED_3D || $pr->ID_SHAPE == CYLINDER_LAYING_3D || $pr->ID_SHAPE == SPHERE_3D || $pr->ID_SHAPE == OVAL_STANDING_3D || $pr->ID_SHAPE == OVAL_LAYING_3D) {
                    if ($key < $count - 1) {
    ...
    public function removeProductElement($id)
    {
        ...
        $conf = $this->kernel->getConfig($this->auth->user()->ID_USER, $element->product->ID_STUDY, -1, 1, 1, 'c:\\temp\\'.$study->STUDY_NAME.'\\StudyCleaner_'.$study->ID_STUDY.'_41_.txt');
        $this->kernel->getKernelObject('StudyCleaner')->SCStudyClean($conf, SC_CLEAN_OUTPUT_PRODUCT);
        ...
        //run studyCleaner 41
        $conf = $this->kernel->getConfig($this->auth->user()->ID_USER, $studyId, -1, 1, 1, 'c:\\temp\\'.$study->STUDY_NAME.'\\StudyCleaner_'.$study->ID_STUDY.'_41_.txt');
        $this->kernel->getKernelObject('StudyCleaner')->SCStudyClean($conf, SC_CLEAN_OUTPUT_PRODUCT);
        ...
        $conf = $this->kernel->getConfig($this->auth->user()->ID_USER, intval($id), -1, 1, 1, 'c:\\temp\\'.$study->STUDY_NAME.'\\WeightCalculator_'.$study->ID_STUDY.'_4_.txt');
